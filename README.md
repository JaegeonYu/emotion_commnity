...
1. MVC vs REST
- 템플릿엔진에서 GET, POST HTTP 메소드만 지원하고 나머지는 지원하지 않아서 hid**den으로 메소드를 바꿀때도, Server API를 설계할 때도 추상적으로 알던 REST API랑은 거리가 멀다고 생각했고 HTTP프로토콜, REST API, Spring Web MVC를 공부하면서 MVC패턴을 이용한 서버 사이드 렌더링은 RESTfull 하기 힘든 것을 배웠습니다.
