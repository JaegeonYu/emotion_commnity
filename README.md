...
서버 사이드 렌더링 프로젝트를 끝내고 느낀 바는

### 장점 
#### 서버와 화면이 유기적이지만  생산성이 높다.
- 처음 유저관리를 경험해봤지만 Thymeleaf의 세션id 관리와 Thymeleaf Extras Security로 View Layer의 인가를 손쉽게 구현할 수 있었습니다. View Layer에서 세션관리를 하는 것은 너무 유기적으로 구성된 것은 아닌가했지만 개발자가 모든 것을 관리할 수 있는 작은 규모의 서버 사이드 렌더링에서는 높은 생산성을 가질 수 있다는 장점이 될 수 있다고 생각했습니다.

#### 단점

#### REST API 설계가 어렵다
- Server URL을 설계 할 때 한 가지 행동을 하기위해 폼을 보여주기 위한 부분과, 해당 행동을 처리하는 부분으로 나뉘는 것과 같이 HTTP 답지 못한 URL 설계를 경험했습니다.
- 템플릿 엔진에서 GET, POST HTTP Method만 지원하고 나머지는 지원하지 않아서 hidden으로 메소드를 바꾸는 것 같이 HTTP 답지 못한 Method 사용을 경험햇습니다.
- 컨트롤러는 뷰와 묶여있어서 서비스 Layer만 테스트하고 컨트롤러는 테스트하지 못했고 Rest Docs 같은 API 문서화를 진행하지 못했습니다.
